...
### 17. Tailwind CSS v4 is incompatible with Angular 17
...
### 18. `tryRestoreSession()` must not be called twice on app init
...
### 19. `POST /api/users/me` must be called by the frontend after OAuth login
...
### 20. Local auth tokens must use a different localStorage key than `angular-oauth2-oidc`
